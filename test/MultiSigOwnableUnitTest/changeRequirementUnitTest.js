...
contract('MultiSigOwnable: changeRequirement Unit Test', function(accounts) {
  ...
  it("checks that changeRequirement work as intended if all the condition are valid", async function () {
    ...
    const changeRequirementEncodedDataField = ownableInstance.contract.changeRequirement.getData(3);
  ...
  it("revert if not called from the smart contract Address", async function () {
    const changeRequirementEncodedDataField = ownableInstance.contract.changeRequirement.getData(3);
    await utils.assertRevert(ownableInstance.sendTransaction({from: accounts[0], data: changeRequirementEncodedDataField}));

    const res = await ownableInstance.callSelf(changeRequirementEncodedDataField);
  ...
  it("revert if valid requirements aren't met", async function () {
    ...
    let changeRequirementEncodedDataField = ownableInstance.contract.changeRequirement.getData(ownerList.length + 1);
    await ownableInstance.callSelf(changeRequirementEncodedDataField);
    ...
    changeRequirementEncodedDataField = ownableInstance.contract.changeRequirement.getData(0);
    await ownableInstance.callSelf(changeRequirementEncodedDataField);
