...
contract('SideChain: submitSignature Unit Test', function(accounts) {
  beforeEach(async function () {
    ...
    sigs = utils.multipleSignedTransaction([0,1,2,3,4,5,6], contractAddress, txHash, toAddress, value, data, version);
  });
  ...
  it("checks that a new transaction object is added for first signature submission", async function () {
    ...
    await sidechainInstance.submitSignatureMC(txHash, toAddress, value, data, sigs.v[0], sigs.r[0], sigs.s[0]);
  ...
  it("checks that signature is added properly to existing transaction object for non-first signature submission", async function () {
    ...
    await sidechainInstance.submitSignatureMC(txHash, toAddress, value, data, sigs.v[1], sigs.r[1], sigs.s[1], {from: accounts[1]});
    tx = await sidechainInstance.getTransactionMC.call(txHash);
  ...
  it("revert signature provided is not signed by the caller", async function () {
    ...
    await utils.assertRevert(sidechainInstance.submitSignatureMC(txHash, toAddress, value, data, sigs.v[0], sigs.r[0], sigs.s[0], {from: nonSigner}));

    await sidechainInstance.submitSignatureMC(txHash, toAddress, value, data, sigs.v[0], sigs.r[0], sigs.s[0], {from: signer});
  });

  it("revert if called from a non owner account", async function () {
    // should revert because it is calling from non owner account
    const nonOwner = accounts[9];
    await utils.assertRevert(sidechainInstance.submitSignatureMC(txHash, toAddress, value, data, sigs.v[0], sigs.r[0], sigs.s[0], {from: nonOwner}));

    const owner = accounts[0];
    await sidechainInstance.submitSignatureMC(txHash, toAddress, value, data, sigs.v[0], sigs.r[0], sigs.s[0], {from: owner})
  });
  ...
  it("revert if destination address is null", async function () {
    ...
    await utils.assertRevert(sidechainInstance.submitSignatureMC(txHash, toAddress, value, data, sigs.v[0], sigs.r[0], sigs.s[0]));
  ...
  it("checks that signatureAdded event is emitted properly", async function () {
    const res = await sidechainInstance.submitSignatureMC(txHash, toAddress, value, data, sigs.v[0], sigs.r[0], sigs.s[0]);
