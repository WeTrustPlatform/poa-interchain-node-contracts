...
contract('MainChain: removeBlackList Unit Test', function(accounts) {
  ...
  it("checks that removeBlackList work as intended if all the condition are valid", async function () {
    ...
    const removeBlackListSigs = utils.multipleSignedTransaction([0, 1], contractAddress, txHashes[1], toAddress, value, removeBlackListData, version);
    await mainchainInstance.submitTransaction(txHashes[1], toAddress, value, removeBlackListData, removeBlackListSigs.v, removeBlackListSigs.r, removeBlackListSigs.s, {from: accounts[0]});
  ...
  it("revert if tx is not blacklisted", async function () {
    ...
    const removeBlackListSigs = utils.multipleSignedTransaction([0, 1], contractAddress, txHashes[1], toAddress, value, removeBlackListData, version);
    const res = await mainchainInstance.submitTransaction(txHashes[1], toAddress, value, removeBlackListData, removeBlackListSigs.v, removeBlackListSigs.r, removeBlackListSigs.s);
  ...
  it("checks that UnBlackListed event is emitted properly", async function () {
    const removeBlackListData = mainchainInstance.contract.removeBlackList.getData(txHash);
    const sigs1 = utils.multipleSignedTransaction([0, 1], contractAddress, txHashes[1], toAddress, value, removeBlackListData, version);
    const res = await mainchainInstance.submitTransaction(txHashes[1], toAddress, value, removeBlackListData, sigs1.v, sigs1.r, sigs1.s);
